...
#^{:doc "Runs an sql select statement built from the given select-map. The valid keys are: table - the table to run the select statement on, select - the columns to return, where - the conditions"}
  sql-find [connection table-name select-map]
  (let [select (or (:select select-map) "*")
        where (:where select-map)
        limit (:limit select-map)]
    (execute-query connection 
      (str "SELECT " select " FROM " table-name
           (if where (str " WHERE " where) nil) 
           (if limit (str " LIMIT " limit) nil)))))
...
#^{:doc "Deletes the table with the given name."}
  drop-table [connection table-name]
  (execute-update 
    connection (str "DROP TABLE " table-name )))

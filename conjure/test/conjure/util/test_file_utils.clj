(ns conjure.util.test-file-utils
...
(deftest test-user-directory
  (let [user-dir (user-directory)]
    ...
    (is (= (.getName user-dir) "conjure"))))
        
(deftest test-find-file
  (let [parent-dir (File. (user-directory) "test/conjure/util")]
...
(deftest test-write-file-content
  (let [test-content "Test content."
        test-file (new File (user-directory) "test/conjure/util/test.txt")]
